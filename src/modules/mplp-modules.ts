...
export type ModuleId = 'context' | 'plan' | 'confirm' | 'trace' | 'role' | 
...
export type PlanStatus = 'draft' | 'proposed' | 'approved' | 'in_progress' | 'completed' | 'cancelled' | 'failed';
...
export function createConfirm(input: CreateConfirmInput): Confirm {
  ...
  if (!VALID_TARGET_TYPES.includes(input.target_type)) {
    throw new Error(`Confirm: target_type must be one of ${VALID_TARGET_TYPES.join(', ')} (GATE-CONFIRM-TARGETTYPE-01)`);
  }
...
export type ExtensionType = 'capability' | 'policy' | 'integration' | 'transformation' | 'validation' | 'other';
...
export function createExtension(input: CreateExtensionInput): Extension {
  ...
  if (!SEMVER_RE.test(input.version)) {
    throw new Error(`Extension: version must be SemVer format (GATE-EXTENSION-SEMVER-01), got: ${input.version}`);
  }
...
export type TopologyType = 'single_node' | 'hub_spoke' | 'mesh' | 'hierarchical' | 'hybrid' | 'other';
