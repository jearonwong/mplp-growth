...
describe("Phase 19: Queue Search & Filter (v0.6.2)", () => {
  beforeEach(() => {
    ...
    window.fetch = () => Promise.resolve({ json: () => Promise.resolve({}) } as any);
    ...
    try {
      ...
      (window as any).eval(safeJs);
      (window as any).renderQueueList = eval("renderQueueList");
    } catch (e) {
    ...
    window.app.state.queueItems = [
      ...
      },
      {
        ...
        interaction_summaries: [
          ...
          { platform: "manual", author: "bob", excerpt: "second summary", source_ref: "" },
          { platform: "hn", author: "charlie", excerpt: "third summary", source_ref: "" },
        ],
  ...
  it("SEARCH-FILTER-01: search matches title/preview", () => {
    ...
    window.app.handlers.onSearchInput({ target: searchInput } as any);
  ...
  it("SORT-ORDER-01: created_at DESC stable sort mapping", () => {
    ...
    window.app.state.queueItems.sort((a: any, b: any) => {
